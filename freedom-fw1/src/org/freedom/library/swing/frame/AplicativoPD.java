...
public class AplicativoPD extends Aplicativo implements ActionListener, KeyListener {
	...
	public void setInfoTela() throws Exception {
		...
		if (getSgestacao().getDescest()==null) {
			telaPrincipal.statusBar.setDescEst("Poste de travail non enregistré");
		} else {
	...
	public void iniConexao() {
		...
		try {
			...
			Funcoes.mensagemErro(null, "Impossible de charger le paramètre 'codemp'\n" + err.getMessage(), true, con, err);
		}
		...
		try {
			...
			Funcoes.mensagemErro(null, "Impossible de charger le paramètre 'codfilialparam'\n" + err.getMessage(), true, con, err);
		}
		...
		try {
			...
			Funcoes.mensagemErro(null, "Impossible de charger le paramètre 'numterm'\n" + err.getMessage(), true, con, err);
		}

		if (strBanco == null) {
			Funcoes.mensagemInforma(null, "paramètre Banque de n'a pas été rempli");
			return;
		}
		if (strDriver == null) {
			Funcoes.mensagemInforma(null, "Pilote de paramètre n'a pas été achevé");
			return;
		}
		...
		try {
			...
			Funcoes.mensagemErro(null, "Impossible de charger le paramètre 'fbversao'\n" + err.getMessage(), true, con, err);
		}
	...
	public static int[] gravaLog(String sIDUSU, String sClas, String sTipo, String sDesc, String sObs, DbConnection con) {
		...
		try {
			...
			Funcoes.mensagemErro(null, "Erreur d'écriture LOG!!\n" + err.getMessage(), true, con, err);
			err.printStackTrace();
		}
		return iRet;
	}
	...
	private void loadSgestacao() throws Exception {
		if (getSgestacao()==null) {
			...
			try {
				...
				throw new Exception("Erreur lors du chargement de module!\n"+mensagemErr);
			}
			setSgestacao(sgestacao);
			if (sgestacao.getCodest()==null) {
				throw new Exception("module non enregistré !");
			}
		}
	}
